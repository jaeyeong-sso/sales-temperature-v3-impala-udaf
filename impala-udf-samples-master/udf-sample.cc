...
BigIntVal DivideUdf(FunctionContext* context, const BigIntVal& totalAmount, const IntVal& totalCount){
  if(totalAmount.is_null || totalCount.is_null) return BigIntVal::null();
  return BigIntVal(totalAmount.val / totalCount.val);
}
