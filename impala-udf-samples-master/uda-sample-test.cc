...
bool TestAvgSalesVolume(){
  UdaTestHarness2<StringVal, StringVal, DoubleVal, IntVal> test(
     AvgSalesVolumeInit, AvgSalesVolumeUpdate, AvgSalesVolumeMerge, AvgSalesVolumeSerialize, AvgSalesVolumeFinalize);

  vector<DoubleVal> vals_of_double;
  vector<IntVal> vals_of_int;

  vals_of_double.push_back(1000);
  vals_of_double.push_back(1000);
  vals_of_double.push_back(1000);

  vals_of_int.push_back(2);
  vals_of_int.push_back(2);
  vals_of_int.push_back(2);

  const char* expected_charptr = "300";
  StringVal expected_stringval(expected_charptr);

  // Test empty input
  if (!test.Execute(vals_of_double, vals_of_int, expected_stringval)) {
    cerr << "Result of AvgSalesVolume : " << test.GetErrorMsg() << endl;
    return false;
  }
