...
void AvgSalesVolumeInit(FunctionContext* context, StringVal* val) {
  val->is_null = false;
  val->len = sizeof(AvgSalesVolumeStruct);
  val->ptr = context->Allocate(val->len);
  memset(val->ptr, 0, val->len);
}

void AvgSalesVolumeUpdate(FunctionContext* context, const DoubleVal& input, const IntVal& input_day_idx,  StringVal* val) {
  if (input.is_null) return;
  if (input_day_idx.is_null) return;

  assert(!val->is_null);
  assert(val->len == sizeof(AvgSalesVolumeStruct));
  AvgSalesVolumeStruct* avg = reinterpret_cast<AvgSalesVolumeStruct*>(val->ptr);
  avg->sum += input.val;

  int read_bit_map = avg->day_bit_mask & (0x0001<<input_day_idx.val);
  if( read_bit_map == 0 ){
     ++avg->count;
     avg->day_bit_mask |= (0x0001<<input_day_idx.val);
  }
}

void AvgSalesVolumeMerge(FunctionContext* context, const StringVal& src, StringVal* dst) {
  if (src.is_null) return;
  const AvgSalesVolumeStruct* src_avg = reinterpret_cast<const AvgSalesVolumeStruct*>(src.ptr);
  AvgSalesVolumeStruct* dst_avg = reinterpret_cast<AvgSalesVolumeStruct*>(dst->ptr);
  dst_avg->sum += src_avg->sum;
  dst_avg->count += src_avg->count;
  dst_avg->day_bit_mask |= src_avg->day_bit_mask;
}
...
// AvgInit().
const StringVal AvgSalesVolumeSerialize(FunctionContext* context, const StringVal& val) {
  assert(!val.is_null);
...
StringVal AvgSalesVolumeFinalize(FunctionContext* context, const StringVal& val) {
  assert(!val.is_null);
  assert(val.len == sizeof(AvgSalesVolumeStruct));
  AvgSalesVolumeStruct* avg = reinterpret_cast<AvgSalesVolumeStruct*>(val.ptr);
  StringVal result;
